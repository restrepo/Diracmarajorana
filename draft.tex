...
The rest of the paper is organized as follows. In the next Section we present the model and study the scalar mass spectrum after spontaneous symmetry breaking. In Sec.~\ref{sec:Neutrinos} we present the radiative mechanism to generate Dirac neutrino masses and establish  the lepton flavor violation constraints. The different DM scenarios are discussed in Sec.~\ref{sec:DM}. Finally, in Sec.~\ref{sec:CosmoConstraints} we show the cosmological restrictions ($N_{\text{eff}}$) in a non-minimum model for different extra abelian symmetries.
