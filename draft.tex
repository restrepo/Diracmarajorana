...
\end{align}
For example, the solution with $\lambda=3$ $n_{\alpha}=\left( -2,-2,-4,-1 \right)$~\cite{Appelquist:2002mw} can be easily normalized to the form \eqref{eq:anolamnor} wiht $n_{\alpha}\to n_{\alpha}/3$ as used in~\cite{Patra:2016ofq}. We work with the normalized solution in terms of a single parameter~\cite{Jenkins:1987ue,Oda:2015gna,Okada:2018tgy}, that we choose to be $l$, as summarized in column $\operatorname{U}(1)_X$ of Table~\ref{tab:partcont3}. In particular, this includes the solution  $n_{\alpha}=(-4,-4,+5)$~\cite{Appelquist:2002mw}. 
