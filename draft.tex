...
terms at tree level.
%Additionally, we add three right neutrinos, one scalar doublet, two scalar singlets and three Majorana fermion singlet under $SU(2)_L$.
...
%
\begin{align*}
  H = \begin{pmatrix}G^{\pm} \\ \frac{1}{\sqrt{2}} (\varphi+v_H+iG) \end{pmatrix} \,,&\hspace{1cm}
...
\end{align}
If we work in the basis where $\ell$ and $\nu_{R}$ are mass eigenstates, the matrices $U_{L}^{(\ell)} = U_{R}^{\nu} = \mathds{1}$. Therefore, we can express the Dirac neutrino mass matrix in the following way
\begin{align}
