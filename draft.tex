...
\end{align*}
%
where $L_{i}$ ($i=1,2,3$) and $H$ are the SM lepton and Higgs doublets, respectively,  $\widetilde{\eta} = i \sigma_2 \eta^*$, and $h$, $y$ and $\kappa$ are matrices in the flavor space. 
...
\section{Dark matter}
\label{sec:DM}
From the charge assignment presented in Table~\ref{tab:partcont} we have that a residuar $Z_2$ symmetry is left over after the $\operatorname{U}(1)_X$ symmetry breaking, with the particles circulating the one-loop neutrino mass diagram  (see Fig.~\ref{fig:zee}) and $\nu_{R3}$ being odd under it whereas $\nu_{R1}, \nu_{R2}$ and and all the SM particles are even. Thus the lightest neutral $Z_2$-odd particle can be a dark matter candidate. In other words, this model features either fermion ($\psi_1$) or scalar ($\chi_{1R}$ or $\chi_{1I}$) dark matter\footnote{Note that $Z'$ is not stable because the $Z'\to\bar{\nu}_{R3}\nu_{R3}$ decay channel can not be kinematically closed.}. 

%\subsection{Fermion dark matter}
Since $\psi_{R1}$ is a singlet under the SM gauge group its thermal relic density is controlled by the Yukawa and $\operatorname{U}(1)_X$ gauge interactions. 
...
In this case sizable $h_{i\alpha}$ are required to reproduce the correct DM abundance, which in turn leads to a mild tension with experimental upper bounds on the rates for rare charged lepton decays. 
The phenomenology of this scenario has been extensively studied  \cite{Kubo:2006yx,Sierra:2008wj,Suematsu:2009ww,Gelmini:2009xd,Hambye:2009pw,Aoki:2010tf,Schmidt:2012yg,Ma:2012if,Kashiwase:2012xd,Kashiwase:2013uy,Racker:2013lua,Toma:2013zsa,Molinaro:2014lfa,Vicente:2014wga,Merle:2015gea,Merle:2015ica,Merle:2016scw,Hagedorn:2016dze,Lindner:2016kqk,Hessler:2016kwm}.
On the other hand, when the $Z'$ portal is the main gate to visible sector, $\psi_1$ largely annihilates  into neutrinos (see~\cite{Blanco:2019hah} for a recent review on $Z'$-mediated Majorana DM). 
% check Ibarra papers
...
this model~\cite{}, the relevant c


oupling for DM phenomenology is the
Higgs portal trough the Lagrangian


 term $\sigma^{2} H^{2}_{i}$.
...
In this case we can use the solutions to the anomaly cancellation conditions found for an Abelian gauge symmetry $\operatorname{U(1)_X}$ in~\cite{Appelquist:2002mw,Campos:2017dgc,Okada:2018tgy,Jana:2019mez}.
...
$\sigma$ & $\boldsymbol{1}$ & $0$ & $13/4$& $13/4$&  $13/4$ & $13/4$& $13/4$ \\
\hline
