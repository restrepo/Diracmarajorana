...
We propose two Higgs doublets model based on the scotogenic model. In this model neutrino masses are radiative and we assume that neutrinos are Dirac fermion. Viable candidates for dark matter are the mediators in the loop. In addition, we propose that the fermionic mediator in the loop be Majorana fermion. This model is restricted by observables of neutrino physics,  lepton flavor violation and cosmological constrain in the number of relativistic degrees of freedom for neutrinos. Here we find that $M_{Z^{\prime}}/g^{\prime} \gtrsim 60$TeV in order to satisfy the conditions of $N_{eff}$..
\end{abstract}
...
\begin{figure}
\centering
\includegraphics[scale=0.6]{Figures/Tdec.pdf}
\includegraphics[scale=0.6]{Figures/Delta_Neff1.pdf}
\caption{One-loop generation of Dirac neutrino mass}
\label{fig:Neff}
\end{figure}
%
