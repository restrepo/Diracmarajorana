...
\begin{align}
\label{eq:LagY}
    \mathcal{L} \supset& -\,g'\,Z_\mu^\prime\sum_{F}q_{F}\overline{F} \gamma^\mu F+\sum_{\phi}\left|\left( \partial_\mu +i\,g'\,q_\phi\,Z'_\mu \right) \phi\right|^2\nonumber\\
    &-[ 
...
\end{align}
%
In the first row $g'$ is the gauge coupling associated to the $\operatorname{U}(1)_X$ group and $Z_\mu^\prime$ is its corresponding gauge boson, $F$ ($\phi$) denote the new fermions (scalars), and $q_{F,\,\phi}$ their $X$ charges. In the second row
...
On the other hand, we assume that the heavy Majorana fermions are already in the diagonal basis in such a way their masses are $M_{\psi_\alpha}=\kappa_{\alpha}v_S=\sqrt{2}\kappa_{\alpha\alpha}v_S$, with $M_{\psi_1}<M_{\psi_2}<M_{\psi_3}$. Finally, the mass of the new gauge boson is given by $M_{Z'}=q_Sg'v_S$.
%\begin{pmatrix}
  %\psi_1 &
  %\psi_2 &
  %\psi_3
...
In this case we can use the solutions to the anomaly cancellation conditions found for an Abelian gauge symmetry $\operatorname{U(1)}_X$ in Refs.~\cite{Jenkins:1987ue,Appelquist:2002mw,Campos:2017dgc,Okada:2018tgy,Jana:2019mez}, where the previous dark symmetry corresponds to the fermiophobic setup  $X=N$~\cite{Campos:2017dgc,Bertuzzo:2018itn,Bertuzzo:2018ftf}.
...
$Q $     & $\boldsymbol{2}$ & $-1/6$ & $-l/3$   & $1/3$&    $0$&  $1/2$&  $1/6$ \\
$e_R $   & $\boldsymbol{1}$ & $-1$   & $1+2l$   &  $-1$&    $1$ &  $-2$&  $0$ \\\hline
...
the three right neutrinos within the not-so minimal model
...
The results for $\Delta N_{\text{eff}}$ as a function of $M_{Z^{\prime}}/g^{\prime}$ are displayed in Fig.~\ref{fig:Neff}. We can see that for small decoupling temperatures, the variation in the number of relativistic degrees of freedom of the new species is large and vice versa.
...
\begin{figure}
\centering
\includegraphics[scale=1.2]{D_Neff.pdf}
...
\end{align}
which is obtained from the search for effective four-lepton operators and is valid for $M_{Z'}\gg 200\ \text{GeV}$. This constraint is relevant for $M_{Z'}>5.8\ \text{TeV}$, and corresponds to the excluded magenta (lower) region of fig.~\ref{fig:u1blc}.
