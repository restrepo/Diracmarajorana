...
With the aim of thermalize the right-handed neutrinos we consider an extension of the previous setup by introducing a general Abelian gauge symmetry $\operatorname{U(1)}_X$ with generation-independent charge assignments for the SM fields. 

additional chiral SM singlet fields and assigning nonzero $\operatorname{U}(1)_X$ charges to the
%A not-so-minimal model can be obtained by introducing additional chiral SM singlet fields. 
In this case we can use the solutions to the anomaly cancellation conditions found for a general   in Refs.~\cite{Jenkins:1987ue,Appelquist:2002mw,Campos:2017dgc,Oda:2015gna,Okada:2018tgy,Jana:2019mez}, where the previous dark symmetry denoted as $X=\mathcal{D}$ corresponds to the fermiophobic setup~\cite{Campos:2017dgc,Bertuzzo:2018itn,Bertuzzo:2018ftf}.
...
dimension-6 operator at one-loop have vector-like $X$-charges with a new set of $\psi_{L\alpha}$ fermions. Otherwise, the anomaly cancellation conditions would be spoiled.
