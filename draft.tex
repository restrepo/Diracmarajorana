...
\end{align}
We choose the solutions with $\lambda\equiv e-2l$, such that
\begin{align}
  \label{eq:anolam}
  \sum_{\alpha=1}^{n} n_{\alpha}=&-3 \lambda\,,   & \sum_{\alpha=1}^{n} n_{\alpha}^3=&-3 \lambda^3\,, 
\end{align}
The full set of standard model charges in terms of two parameters~\cite{Appelquist:2002mw,Campos:2017dgc} that we choose as $l$ and $\lambda$, is just
\begin{align}
  u=&-\lambda-\frac{4l}{3}\,,&d=&\lambda+\frac{2l}{3}\,,&q=&-\frac{l}{3}\,,&e=&\lambda+2l\,,&h=&-\lambda-l\,.
  \label{Eq:SMCharges}
\end{align}
If we now change $f\to  f/\lambda$, such that $\lambda\to 1$, the first of anomaly cancellation~\eqref{eq:anolin} remain invariant, and without lost of generality
always is possible to normalize the solutions such that the last set~\eqref{eq:anolam} is just
\begin{align}
  \label{eq:anolamnor}
