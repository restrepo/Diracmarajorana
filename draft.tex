...
%Dirac case
If neutrinos are Dirac particles, the
Standard Model (SM) particle content must be extended with
right-handed neutrinos,
%with cosmology implications
which can increase the effective number of
light neutrinos, $N_{\text{eff}}$, until 6. Therefore, to be compatible with the
current cosmological restrictions on $N_{\text{eff}}$, the interactions
...
with heavy fermions is usually invoked.
%tree-level case
For the tree-level type-I
...
%one-loop case
The
radiative type-I seesaw includes both~\cite{} possibilities~\cite{},
